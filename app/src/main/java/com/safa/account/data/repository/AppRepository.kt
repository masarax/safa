...
class AppRepository(private val api: ApiService) {
    ...
    suspend fun insertCustomer(c: Customer): Int { val r = api.createCustomer(cp(c)); check(r.isSuccessful); refreshAll(); return responseId(r.body()) }
    suspend fun updateCustomer(c: Customer) { check(api.updateCustomerApi(c.serverId.takeIf { it > 0 } ?: c.id, cp(c)).isSuccessful); refreshAll() }
    ...
    suspend fun insertWalletLedger(l: WalletLedger): Int { val r = api.createWalletLedger(lp(l)); check(r.isSuccessful); refreshAll(); return responseId(r.body()) }
    suspend fun updateWalletLedger(l: WalletLedger) { check(api.updateWalletLedger(l.serverId.takeIf { it > 0 } ?: l.id, lp(l)).isSuccessful); refreshAll() }
