...
class AppRepository private constructor(private val api: ApiService, private val localStore: LocalFirstStore?) {
    ...
    private fun customer(m: Map<String, Any?>) = Customer(id = m.v("local_id", "id").i(), serverId = m.v("server_id", "id").i(), name = m.v("name").s(), phone = m.v("phone").s(), address = m.v("address").s(), securityNotes = m.v("security_notes").s(), avatarColor = m.v("avatar_color").s().ifBlank { "4280391411" }, avatarEmoji = m.v("avatar_emoji").s().ifBlank { "👤" }, timestamp = m.v("timestamp").i().toLong().takeIf { it > 0 } ?: System.currentTimeMillis(), deletedAt = m.v("deleted_at").let { it?.toString()?.toLongOrNull() })
    private fun supplier(m: Map<String, Any?>) = Supplier(id = m.v("local_id", "id").i(), serverId = m.v("server_id", "id").i(), name = m.v("name").s(), phone = m.v("phone").s(), address = m.v("address").s(), tradeLicense = m.v("trade_license").s(), securityNotes = m.v("security_notes").s(), avatarColor = m.v("avatar_color").s().ifBlank { "4280391411" }, avatarEmoji = m.v("avatar_emoji").s().ifBlank { "🏢" }, timestamp = m.v("timestamp").i().toLong().takeIf { it > 0 } ?: System.currentTimeMillis(), deletedAt = m.v("deleted_at").let { it?.toString()?.toLongOrNull() })
    ...
    private fun cp(c: Customer) = mapOf("name" to c.name, "phone" to c.phone, "avatar_color" to c.avatarColor, "avatar_emoji" to c.avatarEmoji, "address" to c.address, "security_notes" to c.securityNotes, "local_id" to c.id, "timestamp" to c.timestamp, "deleted_at" to c.deletedAt)
    private fun sp(s: Supplier) = mapOf("name" to s.name, "phone" to s.phone, "avatar_color" to s.avatarColor, "avatar_emoji" to s.avatarEmoji, "address" to s.address, "trade_license" to s.tradeLicense, "security_notes" to s.securityNotes, "local_id" to s.id, "timestamp" to s.timestamp, "deleted_at" to s.deletedAt)
