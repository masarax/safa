...
interface ApiService {
    ...
    /** Bounded baseline page used only when a device has no valid delta cursor. */
    @GET("sync/down")
    suspend fun syncDownPage(
        @Query("page") page: Int,
        @Query("per_page") perPage: Int = 100,
        @Query("snapshot_cursor") snapshotCursor: Long? = null
    ): Response<SyncDownResponse>

    /** Incremental account-scoped reconciliation after the initial bootstrap. */
    @GET("sync/changes")
    suspend fun syncChanges(
        @Query("cursor") cursor: Long,
        @Query("limit") limit: Int = 100
    ): Response<SyncDeltaResponse>
