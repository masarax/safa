...
class LocalFirstStore(context: Context) : SQLiteOpenHelper(context.applicationContext, "safa_local.db", null, VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        putMeta(db, "device_seed", deviceSeed().toString())
        putMeta(db, "next_local_id", deviceSeed().toString())
    }
    ...
    fun upsertRecord(entity: String, localId: Int, serverId: Int, payload: String, syncStatus: Int = PENDING, retryCount: Int = 0, error: String? = null) {
        ...
        val values = ContentValues().apply { put("entity", entity); put("local_id", localId); put("server_id", serverId); put("payload", PayloadCipher.encrypt(payload)); put("sync_status", syncStatus); put("retry_count", retryCount); put("last_error", error); put("updated_at", now) }
    ...
    fun enqueue(entity: String, localId: Int, serverId: Int, operation: String, payload: String) {
        val now = System.currentTimeMillis()
        val values = ContentValues().apply { put("entity", entity); put("local_id", localId); put("server_id", serverId); put("operation", operation); put("payload", PayloadCipher.encrypt(payload)); put("status", OUTBOX_PENDING); put("retry_count", 0); put("next_attempt_at", 0L); putNull("last_error"); put("created_at", now); put("updated_at", now) }
    ...
    fun getReadyOutbox(limit: Int = 50): List<OutboxRecord> = readableDatabase.rawQuery("SELECT id,entity,local_id,server_id,operation,payload,status,retry_count,last_error FROM outbox WHERE status=? AND next_attempt_at<=? AND retry_count<5 ORDER BY id LIMIT ?", arrayOf(OUTBOX_PENDING, System.currentTimeMillis().toString(), limit.toString())).use { c -> buildList { while (c.moveToNext()) add(OutboxRecord(c.getLong(0), c.getString(1), c.getInt(2), c.getInt(3), c.getString(4), PayloadCipher.decrypt(c.getBlob(5)), c.getString(6), c.getInt(7), c.getString(8))) } }
    fun markOutboxProcessing(ids: List<Long>) { if (ids.isEmpty()) return; writableDatabase.transaction { ids.forEach { execSQL("UPDATE outbox SET status=?, updated_at=? WHERE id=?", arrayOf(OUTBOX_PROCESSING, System.currentTimeMillis(), it)) } } }
    ...
    fun clearEntity(entity: String) { writableDatabase.delete("records", "entity=?", arrayOf(entity)); writableDatabase.delete("outbox", "entity=?", arrayOf(entity)) }
...
private object PayloadCipher {
    private const val ALIAS = "safa_local_payload_key_v1"; private const val TRANSFORMATION = "AES/GCM/NoPadding"; private const val KEYSTORE = "AndroidKeyStore"; private const val IV_SIZE = 12; private const val TAG_SIZE = 128
    ...
    fun decrypt(value: ByteArray): String { try { val raw = Base64.decode(value, Base64.NO_WRAP); val cipher = Cipher.getInstance(TRANSFORMATION); cipher.init(Cipher.DECRYPT_MODE, key(), GCMParameterSpec(TAG_SIZE, raw.copyOfRange(0, IV_SIZE))); return String(cipher.doFinal(raw.copyOfRange(IV_SIZE, raw.size)), StandardCharsets.UTF_8) } catch (_: Exception) { throw SecurityException("Unable to decrypt SAFA local data") } }
}
