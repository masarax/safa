...
for command_name in tar openssl sha256sum php find sort base64; do
...
# Logo filenames are generated by SAFA from a fixed safe alphabet. Hash ordered
# relative names plus file hashes so unchanged content can reuse its encrypted
# snapshot instead of creating 288 duplicate archives per day.
content_sha="$(
  cd "$SAFA_LOGO_SOURCE_DIR"
...
encrypted_tmp=''
cleanup() { rm -f "$status_tmp" "$encrypted_tmp"; }
trap cleanup EXIT
