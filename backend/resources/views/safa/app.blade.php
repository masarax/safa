...
<head>
    ...
    <title>{{ $setting?->app_name ?: 'SAFA' }}</title>
...
<div id="safa-app"
    ...
    <header class="mobile-topbar">
        <button class="brand-trigger" type="button" data-open-settings aria-label="{{ $bn ? 'সেটিংস' : 'Open settings' }}">
            ...
            <span><strong class="brand-name">{{ $setting?->app_name ?: 'SAFA' }}</strong><small id="signed-user-name">{{ $user->name }}</small></span>
        </button>
        <div class="topbar-actions">
            ...
            <button class="icon-button" type="button" id="theme-toggle" title="{{ $bn ? 'থিম' : 'Theme' }}">◐</button>
            <button class="icon-button" type="button" data-open-settings title="{{ $bn ? 'সেটিংস' : 'Settings' }}">⚙</button>
        </div>
    </header>
    ...
    <main class="mobile-content" id="mobile-content">
        ...
        <section class="screen active" data-screen="dashboard">
            <div class="screen-title"><div><p class="eyebrow">{{ $bn ? 'SAFA' : 'SAFA' }}</p><h1>{{ $bn ? 'ড্যাশবোর্ড' : 'Dashboard' }}</h1></div><button class="compact-button" type="button" data-action="refresh">↻ {{ $bn ? 'রিফ্রেশ' : 'Refresh' }}</button></div>
            <div id="dashboard-stats" class="metric-grid"></div>
            <div class="surface-card"><div class="card-heading"><h2>{{ $bn ? 'আজকের কার্যক্রম' : 'Business overview' }}</h2></div><div id="dashboard-overview" class="overview-grid"></div></div>
        </section>

        @if($canCustomers)
        <section class="screen" data-screen="customers">
            <div class="screen-title"><div><p class="eyebrow">{{ $bn ? 'কাস্টমার তালিকা' : 'Customer directory' }}</p><h1>{{ $bn ? 'কাস্টমার' : 'Customers' }}</h1></div>@if($permissions['can_add_customers'] ?? false)<button class="primary-button" type="button" data-open-create="customer">＋ {{ $bn ? 'নতুন' : 'Add' }}</button>@endif</div>
            <div class="tool-row"><label class="search-box"><span>⌕</span><input id="customer-search" type="search" placeholder="{{ $bn ? 'নাম, ফোন বা ঠিকানা খুঁজুন' : 'Search name, phone or address' }}"></label><select id="customer-filter"><option value="all">{{ $bn ? 'সব' : 'All' }}</option><option value="due">{{ $bn ? 'বকেয়া' : 'Due' }}</option><option value="advance">{{ $bn ? 'অগ্রিম' : 'Advance' }}</option></select><select id="customer-sort"><option value="newest">{{ $bn ? 'নতুন প্রথম' : 'Newest' }}</option><option value="oldest">{{ $bn ? 'পুরাতন প্রথম' : 'Oldest' }}</option><option value="name">A–Z</option><option value="due">{{ $bn ? 'বকেয়া' : 'Due' }}</option></select></div>
        ...
        <section class="screen" data-screen="suppliers">
            <div class="screen-title"><div><p class="eyebrow">{{ $bn ? 'সাপ্লায়ার খাতা' : 'Supplier ledgers' }}</p><h1>{{ $bn ? 'সাপ্লায়ার' : 'Suppliers' }}</h1></div>@if($permissions['can_add_suppliers'] ?? false)<button class="primary-button" type="button" data-open-create="supplier">＋ {{ $bn ? 'নতুন' : 'Add' }}</button>@endif</div>
            <div class="tool-row"><label class="search-box"><span>⌕</span><input id="supplier-search" type="search" placeholder="{{ $bn ? 'নাম, ফোন বা ঠিকানা খুঁজুন' : 'Search supplier' }}"></label><select id="supplier-filter"><option value="all">{{ $bn ? 'সব' : 'All' }}</option><option value="receivable">{{ $bn ? 'পাওনা' : 'Receivable' }}</option><option value="payable">{{ $bn ? 'বকেয়া' : 'Payable' }}</option></select><select id="supplier-sort"><option value="newest">{{ $bn ? 'নতুন প্রথম' : 'Newest' }}</option><option value="oldest">{{ $bn ? 'পুরাতন প্রথম' : 'Oldest' }}</option><option value="name">A–Z</option><option value="balance">{{ $bn ? 'ব্যালেন্স' : 'Balance' }}</option></select></div>
        ...
        <section class="screen" data-screen="wallet">
            <div class="screen-title"><div><p class="eyebrow">{{ $bn ? 'ওয়ালেট লেজার রেজিস্টার' : 'Wallet ledger registers' }}</p><h1>{{ $bn ? 'ওয়ালেট' : 'Wallet' }}</h1></div><button class="primary-button" type="button" data-open-wallet-ledger>＋ {{ $bn ? 'লেজার' : 'Ledger' }}</button></div>
        ...
        <section class="screen" data-screen="expenses">
            <div class="screen-title"><div><p class="eyebrow">{{ $bn ? 'দৈনিক খরচ ও অন্যান্য' : 'Daily operations' }}</p><h1>{{ $bn ? 'আয় / ব্যয়' : 'Income / Expense' }}</h1></div><button class="primary-button" type="button" data-open-expense>＋ {{ $bn ? 'এন্ট্রি' : 'Entry' }}</button></div>
        ...
        <section class="screen sub-screen" data-screen="settings">
            <div class="screen-title"><div><p class="eyebrow">{{ $roleLabel }}</p><h1>{{ $bn ? 'সেটিংস' : 'Settings' }}</h1></div><button class="compact-button" type="button" data-settings-back>← {{ $bn ? 'ফিরুন' : 'Back' }}</button></div>
            <div class="settings-grid">
                <article class="surface-card">
                    <div class="card-heading"><div><h2>{{ $bn ? 'আমার অ্যাকাউন্ট' : 'My account' }}</h2><p>{{ $bn ? 'এগুলো শুধু আপনার নিজের লগইন পরিচয়।' : 'These fields belong only to your signed-in user.' }}</p></div></div>
                ...
                <article class="surface-card">
                    <div class="card-heading"><h2>{{ $bn ? 'ব্যক্তিগত পছন্দ' : 'Personal preferences' }}</h2></div>
                ...
                <article class="surface-card">
                    <div class="card-heading"><h2>{{ $bn ? 'পিন পরিবর্তন' : 'Change PIN' }}</h2></div>
                ...
                <article class="surface-card span-2" id="brand-business-config">
                    <div class="card-heading"><div><h2>{{ $bn ? 'ব্র্যান্ড ও ব্যবসা কনফিগারেশন' : 'Brand & Business Configuration' }}</h2><p>{{ $bn ? 'অ্যাপ ব্র্যান্ড, কারেন্সি ও রেট ফিচার। ব্যবহারকারীর নাম/মোবাইল এখানে নেই।' : 'App branding, currencies and rate features. User identity is intentionally separate.' }}</p></div></div>
                    <form id="config-form" class="form-grid">
                        <label><span>{{ $bn ? 'অ্যাপ নাম' : 'Application name' }}</span><input name="app_name" maxlength="255" value="{{ $setting?->app_name ?: 'SAFA' }}" required></label>
                ...
                <article class="surface-card span-2" id="user-management-settings">
                    <div class="card-heading"><div><h2>{{ $bn ? 'ইউজার ম্যানেজমেন্ট' : 'User Management' }}</h2><p>{{ $bn ? 'অন্য ব্যবহারকারী ও তাদের অ্যাক্সেস স্তর পরিচালনা করুন।' : 'Manage other users and their role-based access.' }}</p></div></div>
    ...
    <nav class="mobile-bottom-nav" aria-label="{{ $bn ? 'প্রধান নেভিগেশন' : 'Primary navigation' }}">
        <button class="bottom-nav-item active" type="button" data-nav="dashboard"><span>⌂</span><small>{{ $bn ? 'হোম' : 'Home' }}</small></button>
        @if($canCustomers)<button class="bottom-nav-item" type="button" data-nav="customers"><span>♙</span><small>{{ $bn ? 'কাস্টমার' : 'Customers' }}</small></button>@endif
        @if($canSuppliers)<button class="bottom-nav-item" type="button" data-nav="suppliers"><span>▣</span><small>{{ $bn ? 'সাপ্লায়ার' : 'Suppliers' }}</small></button>@endif
        @if($canWallet)<button class="bottom-nav-item" type="button" data-nav="wallet"><span>▰</span><small>{{ $bn ? 'ওয়ালেট' : 'Wallet' }}</small></button>@endif
        @if($canExpenses)<button class="bottom-nav-item" type="button" data-nav="expenses"><span>↕</span><small>{{ $bn ? 'আয়/ব্যয়' : 'Expenses' }}</small></button>@endif
    </nav>
    ...
    <template id="create-customer-template"><form class="stack-form" data-create-entity="customers"><div class="subpage-toolbar"><button type="button" class="icon-button" data-close-modal>←</button><div><p class="eyebrow">{{ $bn ? 'কাস্টমার' : 'Customer' }}</p><h2>{{ $bn ? 'নতুন কাস্টমার যোগ' : 'Add New Customer Profile' }}</h2></div></div><label><span>{{ $bn ? 'নাম' : 'Customer name' }}</span><input name="name" required maxlength="255"></label><label><span>{{ $bn ? 'মোবাইল' : 'Phone' }}</span><input name="phone" inputmode="tel" maxlength="50" required></label><label><span>{{ $bn ? 'ঠিকানা' : 'Address' }}</span><input name="address" maxlength="500"></label><button class="primary-button" type="submit">{{ $bn ? 'সংরক্ষণ' : 'Save' }}</button></form></template>
    <template id="create-supplier-template"><form class="stack-form" data-create-entity="suppliers"><div class="subpage-toolbar"><button type="button" class="icon-button" data-close-modal>←</button><div><p class="eyebrow">{{ $bn ? 'সাপ্লায়ার' : 'Supplier' }}</p><h2>{{ $bn ? 'নতুন সাপ্লায়ার যোগ' : 'Add New Supplier Profile' }}</h2></div></div><label><span>{{ $bn ? 'প্রতিষ্ঠানের নাম' : 'Supplier or business name' }}</span><input name="name" required maxlength="255"></label><label><span>{{ $bn ? 'মোবাইল' : 'Phone' }}</span><input name="phone" inputmode="tel" maxlength="50" required></label><label><span>{{ $bn ? 'ঠিকানা' : 'Address' }}</span><input name="address" maxlength="500"></label><button class="primary-button" type="submit">{{ $bn ? 'সংরক্ষণ' : 'Save' }}</button></form></template>
</div>
</body>
</html>
