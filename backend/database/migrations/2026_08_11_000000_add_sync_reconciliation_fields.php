...
return new class extends Migration
{
    ...
    public function up(): void
    {
        foreach ($this->entities as $table) {
            ...
            Schema::table($table, function (Blueprint $table) {
                if (!Schema::hasColumn($table->getTable(), 'sync_version')) {
                    $table->unsignedBigInteger('sync_version')->default(0)->after('timestamp');
                }
                if (!Schema::hasColumn($table->getTable(), 'last_mutation_id')) {
                    $table->string('last_mutation_id', 128)->nullable()->after('sync_version');
                    $table->index('last_mutation_id');
                }
            });
        }
    ...
    public function down(): void
    {
        ...
        foreach ($this->entities as $table) {
            ...
            Schema::table($table, function (Blueprint $table) {
                if (Schema::hasColumn($table->getTable(), 'last_mutation_id')) {
                    $table->dropIndex([$table->getTable() . '_last_mutation_id_index']);
                    $table->dropColumn('last_mutation_id');
                }
                if (Schema::hasColumn($table->getTable(), 'sync_version')) {
