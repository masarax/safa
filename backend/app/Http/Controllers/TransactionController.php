...
class TransactionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'transactions' => Transaction::where('account_id', $context['account_id'])->whereNull('deleted_at')->orderByDesc('timestamp')->orderByDesc('id')->get(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'amount_sar' => 'nullable|numeric|min:0', 'amount' => 'nullable|numeric|min:0',
            'customer_id' => 'nullable|integer|min:0', 'supplier_id' => 'nullable|integer|min:0',
            'customer_rate' => 'nullable|numeric|min:0', 'supplier_rate' => 'nullable|numeric|min:0',
            'amount_bdt' => 'nullable|numeric|min:0', 'receiver_name' => 'nullable|string|max:255',
            'receiver_phone' => 'nullable|string|max:50', 'receiver_account_type' => 'nullable|string|max:50',
            'receiver_account_no' => 'nullable|string|max:100', 'wallet_batch_id' => 'nullable|integer|min:0',
            'notes' => 'nullable|string', 'local_id' => 'nullable|integer|min:1', 'timestamp' => 'nullable|integer|min:1',
            'type' => 'nullable|string|max:20', 'hash' => 'nullable|string|max:255',
        ]);
        ...
        $transaction = Transaction::withTrashed()->updateOrCreate(
            ...
            [
                ...
                'customer_rate' => (float) ($request->input('customer_rate') ?: 0),
                'supplier_rate' => (float) ($request->input('supplier_rate') ?: 0),
                'amount_bdt' => (float) ($request->input('amount_bdt') ?: 0),
    ...
    public function update(Request $request, $id)
    {
        $context = $this->resolveAuthorizedAccountContext($request);
        if (isset($context['error'])) return $context['error'];
        $transaction = Transaction::withTrashed()->where('account_id', $context['account_id'])->where(function ($q) use ($id) {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'amount_sar' => 'nullable|numeric|min:0', 'amount' => 'nullable|numeric|min:0', 'customer_rate' => 'nullable|numeric|min:0',
            'supplier_rate' => 'nullable|numeric|min:0', 'amount_bdt' => 'nullable|numeric|min:0',
            'receiver_name' => 'nullable|string|max:255', 'receiver_phone' => 'nullable|string|max:50',
            'receiver_account_type' => 'nullable|string|max:50', 'receiver_account_no' => 'nullable|string|max:100',
            'wallet_batch_id' => 'nullable|integer|min:0', 'notes' => 'nullable|string', 'timestamp' => 'nullable|integer|min:1',
        ...
        foreach (['customer_id','supplier_id','wallet_batch_id'] as $field) {
            if ($request->has($field)) $transaction->{$field} = (int) ($request->input($field) ?: 0);
        }
        foreach (['amount_sar','customer_rate','supplier_rate','amount_bdt'] as $field) {
            if ($request->has($field)) $transaction->{$field} = (float) ($request->input($field) ?: 0);
        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if (!$request->boolean('confirmed')) return response()->json([
            ...
            'message' => 'Confirmation required before deleting transaction.',
            'requires_confirmation' => true,
        ], 409);

        $transaction = Transaction::where('account_id', $context['account_id'])->where(function ($q) use ($id) {
            $q->where('id', (int) $id)->orWhere('local_id', (int) $id);
        })->first();
        if (!$transaction) return response()->json(['status' => 'error', 'message' => 'Transaction not found.'], 404);
        $transaction->delete();
