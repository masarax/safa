...
class SyncController extends Controller
{
    ...
    {
        $normalize = static function (string $v) use ($scale): array { [$w, $f] = array_pad(explode('.', $v, 2), 2, ''); return [ltrim($w, '0') ?: '0', str_pad(substr($f, 0, $scale), $scale, '0')]; };
    ...
    public function syncUp(Request $request)
    {
        try {
            ...
            foreach($entities as $entity=>$config) foreach($config['rows'] as $row){if(!is_array($row)){$rejected[]=['entity'=>$entity,'local_id'=>0,'reason'=>'Invalid record','code'=>'VALIDATION'];continue;}try{$result=$this->reconciliation->apply($accountId,$entity,$config['model'],$row,$config['attributes'],$config['validate']??null);}catch(\Throwable $e){Log::warning('Sync record rejected.', ['entity'=>$entity,'local_id'=>(int)($row['local_id']??0),'exception'=>get_class($e)]);$result=['status'=>'rejected','rejected'=>['entity'=>$entity,'local_id'=>(int)($row['local_id']??0),'reason'=>$e->getMessage(),'code'=>$e instanceof \RuntimeException?'DEPENDENCY':'VALIDATION']];}if($result['status']==='accepted')$accepted[$entity][]=$result['accepted'];elseif($result['status']==='conflict')$conflicts[]=$result['conflict'];else$rejected[]=$result['rejected'];}
