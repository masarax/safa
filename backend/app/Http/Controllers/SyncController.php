...
class SyncController extends Controller
{
    ...
    private function compareDecimal(string $a, string $b, int $scale): int
    {
        ...
        $whole = strcmp($aw, $bw); if ($whole !== 0) return $whole <=> 0;
    ...
    public function syncUp(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'transactions' => 'nullable|array|max:500', 'customers' => 'nullable|array|max:500', 'suppliers' => 'nullable|array|max:500',
                'wallet_batches' => 'nullable|array|max:500', 'supplier_deposits' => 'nullable|array|max:500', 'expenses_incomes' => 'nullable|array|max:500', 'wallet_ledgers' => 'nullable|array|max:500',
            ]);
            ...
            $entities = [
                ...
                'supplier_deposits' => ['model' => SupplierDeposit::class, 'rows' => $request->input('supplier_deposits', []), 'attributes' => function (array $r, int $accountId) {
                    $supplierLocal = (int) ($r['supplier_id'] ?? 0); $supplierId = $supplierLocal > 0 ? Supplier::where('account_id', $accountId)->where('local_id', $supplierLocal)->value('id') : 0;
                ...
                'wallet_batches' => ['model' => WalletBatch::class, 'rows' => $request->input('wallet_batches', []), 'attributes' => function (array $r, int $accountId) {
                    $ledgerLocal = (int) ($r['ledger_id'] ?? 0); $supplierLocal = (int) ($r['supplier_id'] ?? 0); $depositLocal = (int) ($r['supplier_deposit_id'] ?? 0);
                    $ledgerId = $ledgerLocal > 0 ? WalletLedger::where('account_id', $accountId)->where('local_id', $ledgerLocal)->value('id') : 0; $supplierId = $supplierLocal > 0 ? Supplier::where('account_id', $accountId)->where('local_id', $supplierLocal)->value('id') : 0; $depositId = $depositLocal > 0 ? SupplierDeposit::where('account_id', $accountId)->where('local_id', $depositLocal)->value('id') : 0;
                ...
                'transactions' => ['model' => Transaction::class, 'rows' => $request->input('transactions', []), 'validate' => fn (array $r) => empty(trim((string) ($r['type'] ?? $r['status'] ?? ''))) ? 'Missing required field: type' : null, 'attributes' => function (array $r, int $accountId) {
                    $customerLocal = (int) ($r['customer_id'] ?? 0); $supplierLocal = (int) ($r['supplier_id'] ?? 0); $batchLocal = (int) ($r['wallet_batch_id'] ?? 0);
                    $customerId = $customerLocal > 0 ? Customer::where('account_id', $accountId)->where('local_id', $customerLocal)->value('id') : 0; $supplierId = $supplierLocal > 0 ? Supplier::where('account_id', $accountId)->where('local_id', $supplierLocal)->value('id') : 0; $batchId = $batchLocal > 0 ? WalletBatch::where('account_id', $accountId)->where('local_id', $batchLocal)->value('id') : 0;
            ...
            $user = $context['user'] ?? $request->user(); $permissions = $user ? $user->getFormattedPermissions() : \App\Models\User::defaultPermissions(true);
        ...
        } catch (\Throwable $e) { Log::error('SyncUp failed.', ['message' => $e->getMessage(), 'exception' => get_class($e)]); return response()->json(['message' => 'Sync failed.'], 500); }
    }
