...
class RemoteBusinessController extends Controller
{
    ...
    {
        return MoneyDecimal::compare($left, $right, 2, 13, false) > 0;
    }
    ...
    {
        $v=Validator::make($request->all(),['supplier_id'=>'sometimes|nullable|integer|min:1','amount_sar'=>['sometimes','required',$this->exactDecimalRule(2,13)],'rate'=>['sometimes','required',$this->exactDecimalRule(4,6)],'amount_bdt'=>['sometimes','required',$this->exactDecimalRule(2,13)],'paid_bdt'=>['sometimes','required',$this->exactDecimalRule(2,13)],'transaction_type'=>'sometimes|string|max:50','notes'=>'sometimes|nullable|string|max:10000','local_id'=>'nullable|integer|min:1','timestamp'=>'nullable|integer|min:1']);
        $v->after(function($validator)use($request,$existing){if($validator->errors()->has('amount_bdt')||$validator->errors()->has('paid_bdt'))return;$amount=$request->has('amount_bdt')?$request->input('amount_bdt'):($existing?->amount_bdt??'0');$paid=$request->has('paid_bdt')?$request->input('paid_bdt'):($existing?->paid_bdt??'0');if($this->decimalGreater($paid,$amount))$validator->errors()->add('paid_bdt','Paid amount cannot exceed the deposit amount.');});return $v;
    }

    public function storeSupplierDeposit(Request $request)
    {
        $c=$this->context($request);if(isset($c['error']))return $c['error'];$v=$this->validateDeposit($request);foreach(['amount_sar','rate','amount_bdt'] as $f)if(!$request->has($f))$v->errors()->add($f,'This field is required.');if($v->errors()->isNotEmpty())return response()->json(['status'=>'error','errors'=>$v->errors()],422);
        ...
        $r=DB::transaction(fn()=>SupplierDeposit::updateOrCreate(['account_id'=>$c['account_id'],'local_id'=>$id],['supplier_id'=>$sid,'amount_sar'=>MoneyDecimal::unsigned($request->input('amount_sar'),2,13),'rate'=>MoneyDecimal::unsigned($request->input('rate'),4,6),'amount_bdt'=>MoneyDecimal::unsigned($request->input('amount_bdt'),2,13),'paid_bdt'=>MoneyDecimal::unsigned($request->input('paid_bdt','0'),2,13),'transaction_type'=>$request->input('transaction_type','SAR_GIVEN'),'notes'=>$request->input('notes'),'timestamp'=>$this->timestamp($request->input('timestamp')),'deleted_at'=>null]));if($r->trashed())$r->restore();return response()->json(['status'=>'success','supplier_deposit'=>$r],201);
    }
    ...
    {
        $v=Validator::make($request->all(),['ledger_id'=>'sometimes|nullable|integer|min:1','rate'=>['sometimes','required',$this->exactDecimalRule(4,6)],'initial_bdt'=>['sometimes','required',$this->exactDecimalRule(2,13)],'remaining_bdt'=>['sometimes','required',$this->exactDecimalRule(2,13)],'supplier_id'=>'sometimes|nullable|integer|min:1','supplier_deposit_id'=>'sometimes|nullable|integer|min:1','notes'=>'sometimes|nullable|string|max:10000','local_id'=>'nullable|integer|min:1','timestamp'=>'nullable|integer|min:1']);
        $v->after(function($validator)use($request,$existing){if($validator->errors()->has('initial_bdt')||$validator->errors()->has('remaining_bdt'))return;$initial=$request->has('initial_bdt')?$request->input('initial_bdt'):($existing?->initial_bdt??'0');$remaining=$request->has('remaining_bdt')?$request->input('remaining_bdt'):($existing?->remaining_bdt??'0');if($this->decimalGreater($remaining,$initial))$validator->errors()->add('remaining_bdt','Remaining amount cannot exceed the initial amount.');});return $v;
    }

    public function storeWalletBatch(Request $request)
    {
        $c=$this->context($request);if(isset($c['error']))return $c['error'];$v=$this->validateBatch($request);foreach(['rate','initial_bdt','remaining_bdt'] as $f)if(!$request->has($f))$v->errors()->add($f,'This field is required.');if($v->errors()->isNotEmpty())return response()->json(['status'=>'error','errors'=>$v->errors()],422);
        ...
        $local=(int)($request->input('local_id')?:floor(microtime(true)*1000));$r=DB::transaction(fn()=>WalletBatch::updateOrCreate(['account_id'=>$c['account_id'],'local_id'=>$local],['ledger_id'=>$lid,'rate'=>MoneyDecimal::unsigned($request->input('rate'),4,6),'initial_bdt'=>MoneyDecimal::unsigned($request->input('initial_bdt'),2,13),'remaining_bdt'=>MoneyDecimal::unsigned($request->input('remaining_bdt'),2,13),'supplier_id'=>$sid,'supplier_deposit_id'=>$did,'notes'=>$request->input('notes'),'timestamp'=>$this->timestamp($request->input('timestamp')),'deleted_at'=>null]));if($r->trashed())$r->restore();return response()->json(['status'=>'success','wallet_batch'=>$r],201);
    }
    ...
    public function updateWalletBatch(Request $request,int $id)
    {
        ...
        foreach([['supplier_id',Supplier::class,'Supplier'],['supplier_deposit_id',SupplierDeposit::class,'Supplier deposit'],['ledger_id',WalletLedger::class,'Wallet ledger']] as [$f,$model,$label])if($request->has($f)){$value=$request->input($f)?(int)$request->input($f):null;if(!$this->relationInAccount($model,$c['account_id'],$value))return response()->json(['status'=>'error','code'=>'DEPENDENCY','message'=>"{$label} does not belong to the active account."],422);$r->{$f}=$value;}if($request->has('rate'))$r->rate=MoneyDecimal::unsigned($request->input('rate'),4,6);foreach(['initial_bdt','remaining_bdt'] as $f)if($request->has($f))$r->{$f}=MoneyDecimal::unsigned($request->input($f),2,13);if($request->has('notes'))$r->notes=$request->input('notes');if($request->has('timestamp'))$r->timestamp=$this->timestamp($request->input('timestamp'));DB::transaction(function()use($r){$r->deleted_at=null;$r->save();});return response()->json(['status'=>'success','wallet_batch'=>$r]);
    }
    ...
    {
        $c=$this->context($request);if(isset($c['error']))return $c['error'];$v=Validator::make($request->all(),['title'=>'required|string|max:255','amount'=>['required',$this->exactDecimalRule(2,13)],'currency'=>'required|string|max:16','is_expense'=>'required|boolean','category'=>'nullable|string|max:255','local_id'=>'nullable|integer|min:1','timestamp'=>'nullable|integer|min:1']);if($v->fails())return response()->json(['status'=>'error','errors'=>$v->errors()],422);$id=(int)($request->input('local_id')?:floor(microtime(true)*1000));$r=DB::transaction(fn()=>ExpenseIncome::updateOrCreate(['account_id'=>$c['account_id'],'local_id'=>$id],['title'=>trim((string)$request->input('title')),'amount'=>MoneyDecimal::unsigned($request->input('amount'),2,13),'currency'=>strtoupper(trim((string)$request->input('currency'))),'is_expense'=>(bool)$request->input('is_expense'),'category'=>$request->input('category'),'timestamp'=>$this->timestamp($request->input('timestamp')),'deleted_at'=>null]));if($r->trashed())$r->restore();return response()->json(['status'=>'success','expense_income'=>$r],201);
    }

    public function updateExpenseIncome(Request $request,int $id)
    {
        $c=$this->context($request);if(isset($c['error']))return $c['error'];$r=$this->findById(ExpenseIncome::class,$c['account_id'],$id);if(!$r)return response()->json(['status'=>'error','message'=>'Expense/income not found.'],404);$v=Validator::make($request->all(),['title'=>'sometimes|required|string|max:255','amount'=>['sometimes','required',$this->exactDecimalRule(2,13)],'currency'=>'sometimes|string|max:16','is_expense'=>'sometimes|boolean','category'=>'sometimes|nullable|string|max:255','timestamp'=>'sometimes|integer|min:1']);if($v->fails())return response()->json(['status'=>'error','errors'=>$v->errors()],422);foreach(['title','currency','is_expense','category'] as $f)if($request->has($f))$r->{$f}=$f==='title'?trim((string)$request->input($f)):($f==='currency'?strtoupper(trim((string)$request->input($f))):$request->input($f));if($request->has('amount'))$r->amount=MoneyDecimal::unsigned($request->input('amount'),2,13);if($request->has('timestamp'))$r->timestamp=$this->timestamp($request->input('timestamp'));DB::transaction(function()use($r){$r->deleted_at=null;$r->save();});return response()->json(['status'=>'success','expense_income'=>$r]);
    }
