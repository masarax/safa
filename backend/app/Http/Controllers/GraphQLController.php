...
class GraphQLController extends Controller
{
        ...
        if (preg_match('/^\s*mutation\b/i', $rawQuery)) {
            return response()->json([
                'errors' => [[
                    ...
                    'extensions' => [
                        'code' => 'GRAPHQL_MUTATIONS_DEPRECATED',
                        'rest_base' => '/api/v1',
                    ],
                ]],
            ], 410);
        ...
        switch ($fieldName) {
            case 'transactions':
                ...
                if (isset($args['customer_id'])) {
                    $query->where('customer_id', $this->positiveId($args['customer_id'], 'customer_id'));
                }
                if (isset($args['supplier_id'])) {
                    $query->where('supplier_id', $this->positiveId($args['supplier_id'], 'supplier_id'));
                }
                if (isset($args['type'])) {
                    $query->where('type', substr((string) $args['type'], 0, 20));
                }
            ...
            case 'wallet_batches':
                ...
                if (isset($args['ledger_id'])) {
                    $query->where('ledger_id', $this->positiveId($args['ledger_id'], 'ledger_id'));
                }
                return $this->boundedRows($query, $limit, $offset, $requestedFields);

            case 'walletLedgers':
            case 'wallet_ledgers':
                return $this->boundedRows(
                    WalletLedger::query()->where('account_id', $accountId)->whereNull('deleted_at'),
                    $limit,
                    $offset,
                    $requestedFields
                );
            ...
            case 'expenses_incomes':
                ...
                if (isset($args['is_expense'])) {
                    $query->where('is_expense', filter_var($args['is_expense'], FILTER_VALIDATE_BOOLEAN));
                }
            ...
            case 'sync_state':
                $latestTransaction = Transaction::query()
                    ...
                    ->first();
                $state = [
                    'lastSyncedAt' => $latestTransaction?->updated_at?->toIso8601String() ?? now()->toIso8601String(),
    ...
    protected function parseArguments(string $rawArgs, array $variables): array
    {
        ...
        if (trim($rawArgs) !== '') {
            preg_match_all(
                '/([a-zA-Z0-9_]+)\s*:\s*(\$[a-zA-Z0-9_]+|"[^"]*"|\'[^\']*\'|true|false|null|[-+]?[0-9]*\.?[0-9]+)/i',
                $rawArgs,
                $matches,
                PREG_SET_ORDER
            );
            foreach ($matches as $match) {
                ...
                if (str_starts_with($raw, '$')) {
                    $value = $variables[substr($raw, 1)] ?? null;
