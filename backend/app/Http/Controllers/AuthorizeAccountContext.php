...
trait AuthorizeAccountContext
{
    protected function resolveAuthorizedAccountContext(Request $request): array
    {
        ...
        if ($requestedAccountId <= 0) {
            ...
            if ($headerAccountId !== null && ctype_digit((string) $headerAccountId)) $requestedAccountId = (int) $headerAccountId;
        }
        ...
        if ($user) {
            if ($requestedAccountId > 0) {
                ...
                $targetAccount = Account::query()->where('owner_user_id', $user->id)->orderBy('id')->first();
                if (!$targetAccount) {
                    $targetAccount = Account::create(['name' => trim(($user->name ?: 'SAFA') . ' Account'), 'balance' => 0, 'owner_user_id' => $user->id]);
                }
            }
